 ...
 *
 */
interface PerformRecipeHandler<Data> {
   type: 'handler'
   message: string
   handler: ContextHandler<Data>
}

interface PerformRecipeSpec<Data> {
   type: 'spec'
   message: string
   spec: Spec<Data>
}

type PerformRecipe<Data> = PerformRecipeHandler<Data> | PerformRecipeSpec<Data>
...
export class Spec<Data = {}> {
   ...
   protected performRecipes: PerformRecipe<Data>[] = []
   ...
   public constructor(stage?: Stage<Data>, parent?: Spec<Data>) {
      this.parent = parent
      this._stage = stage ?? this.createStage()
   }
   ...
   public spec(message: string, spec: Spec<Data>) {
      const specIsKnown =
         this.performRecipes.filter((r) => {
            return r.type === 'spec' && r.spec === spec
         }).length > 0

      if (!specIsKnown) {
         spec.parent = this
         spec.stage = this.stage
         spec.beforeEachHandlers.unshift(...this.beforeEachHandlers)
         spec.afterEachHandlers.push(...this.afterEachHandlers)
      }

      this.performRecipes.push({ type: 'spec', message, spec })
   ...
   public test(message: string, handler: ContextHandler<Data>) {
      this.performRecipes.push({ type: 'handler', message, handler })
      return this
   }
    ...
    *
    */
   protected async performSpec(recipe: PerformRecipeSpec<Data>) {
    ...
    *
    */
   protected async performTest(recipe: PerformRecipeHandler<Data>) {
      const start = Date.now()
